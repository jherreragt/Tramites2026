/*
  ...
  3. Función search_procedures(query text) mejorada:
...
-- Eliminar función anterior y reemplazar con versión mejorada
...
AS $$
  ...
  SELECT
    ...
    (
      SELECT jsonb_agg(
        jsonb_build_object('id', p2.id, 'name', p2.name)
      )
      FROM public.procedures p2
      WHERE p2.institution_id = p.institution_id AND p2.id <> p.id
